...
function placeResults(){
    ...
    for(let iterations=0; iterations<9; iterations++){
            ...
            data.forEach((object)=>{
                Array.from(object.result).forEach((alphabet_B,index_B)=>{
                    if(alphabet_A==alphabet_B){
                        ...
                        let firstAlhabetCellNo = direction=='horizontal' ? intersectCellNo-index_A : intersectCellNo-(index_A*10)
                        placements.push({result:remaining[0], direction, firstAlhabetCellNo})
                    }
                })
            })
        })
        ...
        for(let i=0; i<placements.length; i++){
            let X = cellNoToX(placements[i].firstAlhabetCellNo)
            let Y = cellNoToY(placements[i].firstAlhabetCellNo)
            delete placements [i].firstAlhabetCellNo
...
function arrangeBlocks(){
    ...
    blocks().forEach((block)=>{
        block.style.marginLeft = `${marginTop(block)+(Math.trunc((emptyRowsOnBS-emptyRowsOnUS)/2)*50)}px`
    })
}
